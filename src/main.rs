...
struct Payload {
    ...
    skipped: Vec<String>,
    /// basename -> (mode, content) for payload under /usr/local/bin
    bins: BTreeMap<String, (u32, Vec<u8>)>,
...
fn read_pkg(pkg: &Path, keep_pyc: bool) -> Result<(PkgInfo, Payload)> {
    ...
    let mut bins: BTreeMap<String, (u32, Vec<u8>)> = BTreeMap::new();
    ...
    for entry in ar.entries()? {
        ...
        let Some(rel) = raw.split_once("/site-packages/").map(|(_, r)| r.to_string()) else {
            // bin/ payload is kept aside: entry-point wrappers get dropped
            // (installers regenerate them), real binaries (maturin, ruff)
            // ride along as {name}.data/scripts.
            if entry.header().entry_type().is_file() {
                if let Some(base) = raw.strip_prefix("/usr/local/bin/") {
                    let mode = entry.header().mode().unwrap_or(0o755);
                    let mut buf = Vec::with_capacity(entry.size() as usize);
                    entry.read_to_end(&mut buf)?;
                    bins.insert(base.to_string(), (mode, buf));
                    continue;
                }
                // Docs/licenses/man pages never belong in a wheel; only
                // report payload whose loss could matter.
                let expected_loss =
                    ["/share/doc/", "/share/licenses/", "/share/man/", "/share/examples/"]
                        .iter()
    ...
    Ok((info, Payload { files, skipped, bins, pyver }))
}
...
fn convert(pkg: &Path, outdir: &Path, platform_tag: &str, keep_pyc: bool) -> Result<(PkgInfo, PathBuf)> {
    ...
    // bin/ payload: entry-point wrapper scripts are dropped (regenerated by
    // the installer); anything else — actual binaries like maturin or ruff,
    // possibly under ports' flavored names — ships as .data/scripts so the
    // installer places it in the venv's bin.
    let entry_names: Vec<String> = payload
        .files
        .get(&format!("{distinfo}/entry_points.txt"))
        .map(|(_, data)| {
            ...
            for line in String::from_utf8_lossy(data).lines() {
                let line = line.trim();
                if line.starts_with('[') {
                    in_scripts = line == "[console_scripts]" || line == "[gui_scripts]";
                } else if in_scripts {
        ...
        .unwrap_or_default();
    let data_dir = format!("{}.data", distinfo.trim_end_matches(".dist-info"));
    for (base, (mode, data)) in &payload.bins {
        // ports suffix flavored wrappers with the python version: f2py-3.12
        let stem = base
            .rsplit_once('-')
            .filter(|(_, v)| v.chars().all(|c| c.is_ascii_digit() || c == '.'))
            .map(|(s, _)| s)
            .unwrap_or(base);
        let is_wrapper = data.starts_with(b"#!")
            && (entry_names.iter().any(|n| n == base || n == stem));
        if !is_wrapper {
            write_file(&mut zw, &format!("{data_dir}/scripts/{base}"), *mode | 0o111, data)?;
        }
    }
    ...
    if !payload.skipped.is_empty() {
        eprintln!(
            "note: {} non-site-packages payload file(s) not included:",
            payload.skipped.len()
        );
